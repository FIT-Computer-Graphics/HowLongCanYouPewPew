    ...
    public class EnemyAI : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if(chasing)
        {
            ...
            myTransform.rotation = Quaternion.Slerp(myTransform.rotation, Quaternion.LookRotation(target.position - position1), rotationSpeed * Time.deltaTime);
            ...
            if (distance < minDist)
            {
                ...
                myTransform.rotation = Quaternion.Slerp(myTransform.rotation, Quaternion.LookRotation(position - target.position), rotationSpeed * Time.deltaTime);
    ...
    private void ObstacleAvoidance(Vector3 direction, float offsetX)
    {
        ...
        for(var i = 0; i < hit.Length -1; i++)
        {
            ...
            if (hit[i].transform.root.gameObject == this.gameObject) continue;
            if(!savePos)
            {
    ...
    private void FindEscapeDirections(Collider col)
    {
        ...
        else
        {
            ...
            if (!EscapeDirections.Contains(direct1))
            {
                EscapeDirections.Add(direct1);
            }
        }
        ...
        else
        {
            ...
            if (!EscapeDirections.Contains(direct2))
            {
                EscapeDirections.Add(direct2);
            }
        }
